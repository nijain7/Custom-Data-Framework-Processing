...
//       return data[idx];
        ...
        BST.BSTNode n = seriesDataBST.searchNode(rn);
        return (T)n.getData();
...
//          return cur_dat;
        for(int i = 0; i<rn.length; i++){
            if(seriesDataBST.searchNode(rn[i])==null){
                cur_dat[i] = null;
            }
            else{
                cur_dat[i] = (T)seriesDataBST.searchNode(rn[i]).getData();
            }
        }
        return cur_dat;
...
//               }
        ...
        for (int i = 0; i<dat.length; i++){
            if(dat[i]=="null"){
                ...
                seriesDataBST.updateNode(ind[i],value);
            }
        }
